 ...
 */
function MailComposer(options/*: ?Object */) {
    (Stream/*: any */).call(this);
 ...
 *     <li><b>contentType</b> - content type for the attachment (default will be derived from the filename)</li>
...
MailComposer.prototype.addAlternative = function(alternative) {
    ...
    if (alternative.contents || alternative.streamSource || alternative.filePath) {
 ...
 */
MailComposer.prototype.addAttachment = function(attachment/*: Content */) {
    attachment = attachment || {};

    // Needed for Nodemailer compatibility
    if (attachment.filename) {
        attachment.fileName = attachment.filename;
        delete (attachment/*:any*/).filename;
    }
...
MailComposer.prototype.convertAddress = function(addressObject) {
    // if user part of the address contains foreign symbols
    // make a mime word of it
    var address = addressObject.address.replace(/^.*?(?=\@)/, (function(user) {
    ...
    if (!name) {
        ...
        if (this._hasUTFChars(name)) {
            name = this._encodePartialMimeWord(name, "Q", 52);
        } else {
...
MailComposer.prototype._handleValue = function(key, value) {
    ...
    switch (key) {
        ...
        case "envelope":
            ...
            Object.keys((value/*: any */)).forEach((function(key) {
...
MailComposer.prototype._composeHeader = function() {
    ...
    this._message.tree.headers.forEach(function(header) {
...
/*:: export type MIMETree = AbstractNode | Content */
...
MailComposer.prototype._generateBodyStructure = function() {
    ...
    var textComponent = this._createTextComponent.bind(this);
    var attachmentComponent = this._createAttachmentComponent.bind(this);
    var alternativeComponent = this._createAlternativeComponent.bind(this);
    var childNodes;

    var html = this._message.html;
    var attachments = [];
    var relatedAttachments = [];
    this._attachments.forEach(function(attachment) {
        if (html && attachment.cid && html.indexOf("cid:" + attachment.cid >= 0)) {
            relatedAttachments.push(attachment);
        } else {
    ...
    var htmlPart;
    if (html && relatedAttachments.length > 0) {
        childNodes = relatedAttachments.map(attachmentComponent);
        childNodes.unshift(textComponent(html, "text/html"));
        htmlPart = {
            multipart: "related",
            childNodes: childNodes
        };
    } else if (html) {
        htmlPart = textComponent(html, "text/html");
    }

    var body = this._message.body;
    var textPart = !!body ? textComponent(body, "text/plain") : null;

    var alternatives = [textPart, htmlPart].concat(
        this._alternatives.map(alternativeComponent)
    ).filter(function(a) { return !!a });
    var mainPart = alternatives.length > 1 ?
        { multipart: "alternative", childNodes: alternatives } :
        alternatives[0];

    var topPart = mainPart;
    if (attachments.length > 0) {
        childNodes = attachments.map(attachmentComponent);
        if (mainPart) childNodes.unshift(mainPart);
        topPart = {
            multipart: "mixed",
            childNodes: childNodes
        };
    ...
    topPart = topPart || textComponent("\r\n");

    this._message.tree = this._nodesFromAbstractTree(topPart);
};

/**
 * <p>Manually sets the structure (mime tree) of the body. This sets up
 * multipart structure, individual part headers, boundaries etc.</p>
 *
 * <p>The headers of the root element will be appended to the message
 * headers</p>
 *
 * <p>If no body structure is set by the caller, the
 * <code>_generateBodyStructure</code> method will be called to create one
 * automatically.</p>
 *
 * @param {Object} tree Mime tree root
 */
MailComposer.prototype.setBodyStructure = function(tree/*: MIMETree */) {
    this._message.tree = this._nodesFromAbstractTree(tree);
};

MailComposer.prototype._nodesFromAbstractTree = function(tree/*: MIMETree */)/*: Node */ {
    if (!tree.multipart) {
        return this._createContentNode((tree/*: any */));
    }
    var boundary = this._generateBoundary();
    var multipart/*: AbstractNode */ = (tree/*: any */);
    return {
        childNodes: multipart.childNodes.map(this._nodesFromAbstractTree.bind(this)),
        headers: [["Content-Type", multipartContentType(boundary, multipart)]],
        boundary: boundary,
    };
}

function multipartContentType(boundary/*: string */, node/*: AbstractNode */)/*: string */ {
    return "Content-Type", "multipart/" + node.multipart + "; boundary=\"" + boundary + "\"";
}

/**
 * <p>Creates a content object a text component (plaintext, HTML)</p>
 *
 ...
 */
MailComposer.prototype._createTextComponent = function(
    text/*: string | Buffer */, contentType/*: string */
)/*: Content */ {
    return {
        contentType: contentType || "text/plain",
        contentEncoding: (this.options.encoding || "quoted-printable"),
...
/**
 * <p>Specializes a content object for use as an attachment component</p>
 *
 * @param {Object} attachment Content object: attachment info for the component
 * @return {Object} Content object
 */
MailComposer.prototype._createAttachmentComponent = function(attachment/*: Content */)/*: Node */ {
    return assign({
...
/**
 * <p>Specializes a content object for use as an alternative component (ODF/DOC/etc)</p>
 *
 * @param {Object} alternative Content object: alternative info for the component
 * @return {Object} Content object
 */
MailComposer.prototype._createAlternativeComponent = function(alternative/*: Content */)/*: Content */ {
    return alternative;
};

MailComposer.prototype._createContentNode = function(content/*: Content */)/*: Node */ {
    var fileName = content.fileName ?
        this._encodeMimeWord(content.fileName, "Q", 1024).replace(/"/g, "\\\"") : null;
    var contentEncoding = content.contentEncoding || "base64";

    var headers = [
        this._getContentType(content, contentEncoding, fileName),
        this._getContentDispositon(content, fileName),
        ["Content-Transfer-Encoding", contentEncoding],
        this._getContentId(content)
    ].filter(function(h) { return !!h });

    var node/*: Node */ = {
        childNodes: [],
        headers: headers,
        contentEncoding: (content.contentEncoding || "base64").toLowerCase().trim()
    };

    if (["7bit", "8bit", "binary"].indexOf(contentEncoding) >= 0) {
        node.textFormat = "flowed";
    }

    if (content.contents) {
        node.contents = content.contents;
    } else if (content.filePath) {
        node.filePath = content.filePath;
        if (content.userAgent) {
            node.userAgent = content.userAgent;
        }
    } else if (content.streamSource) {
        node.streamSource = content.streamSource;
    }

    return node;
};

MailComposer.prototype._getContentType = function(
    content/*: Content */,
    contentEncoding/*: string */,
    fileName/*: string */
)/*: ?[string, string] */ {
    var res = parseParameterizedHeader(content.contentType),
        contentType = res[0] || "application/octet-stream";

    var params = assign({
        name:    withQuotes(fileName),
        charset: content.charset,
        format:  ["7bit", "8bit", "binary"].indexOf(contentEncoding) >= 0 ? "flowed" : null,
        method:  content.method
    }, res[1]);

    var withParams = [contentType].concat(
        Object.keys(params)
        .filter(function(k) { return !!params[k] })
        .map(function(k) { return k + "=" + params[k] })
    ).join("; ");

    return ["Content-Type", withParams];
};

MailComposer.prototype._getContentDispositon = function(content/*: Content */, fileName/*: string */)/*: ?[string, string] */ {
    var disp;
    if (content.contentDisposition || fileName) {
        disp = [content.contentDisposition || "attachment"];
        if (fileName) {
            disp.push('filename="' + fileName + '"');
        }
        return ["Content-Disposition", disp.join("; ")];
    }
};

MailComposer.prototype._getContentId = function(content/*: Content */)/*: ?[string, string] */ {
    if (content.cid) {
        return ["Content-Id", "<" + this._encodeMimeWord(content.cid) + ">"];
    }
};

function parseParameterizedHeader(input/*: ?string */)/*: [?string, { [key:string]: string }] */ {
...
function withQuotes(str/*: ?string*/)/*: ?string */ {
    if (str) {
        return '"' + str + '"';
    }
}
    ...
    this._message.html = this._message.html.replace(/<img[^>]*>/gi, (function(imgTag) {
        return imgTag.replace(/\b(src\s*=\s*(?:['"]?))([^'"> ]+)/i, (function(src, prefix, url) {
            ...
            if (url.match(/^https?:\/\//i) || url.match(/^\//i) || url.match(/^[a-z]:\\/i)) {
                cid = (crypto.randomBytes(20)/*: any */).toString("hex") + "@" + this.options.identityString;
                this.addAttachment({
                    filePath: url,
                    cid: cid
                });
